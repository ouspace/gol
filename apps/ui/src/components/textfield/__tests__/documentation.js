...
// ---------- Configuration ----------
const reportsRootDirectory = pathModule.join(process.cwd(), "__tests__", "reports");
...
const jsonSearchCandidates = [
	...
	pathModule.join(process.cwd(), "playwright-report.json"),
	pathModule.join(process.cwd(), "playwright-report", "report.json"),
	pathModule.join(process.cwd(), "test-results.json"),
];
...
function shortPath(p) {
	if (!p) return "unknown";
	return pathModule.relative(process.cwd(), p);
}

// HTML escape utility moved to outer scope
const escapeHtml = (s) =>
...
function generateMarkdown(reportJsonPath, tests, summary) {
	...
	const lines = [
		...
		"",
		`- Generated: **${human}** (${iso})`,
		`- JSON source: \`${pathModule.relative(process.cwd(), reportJsonPath)}\``,
		`- HTML report: \`${pathModule.relative(process.cwd(), htmlReportDirectory)}\``,
		"",
		`## Summary`,
		"",
		`- Total: **${summary.total}**`,
...
function generateHtml(markdownContent) {
	// Simple HTML wrapper. If you want something with brand, add CSS here.
	return `<!doctype html>
<html lang="en">
<head>
	<meta charset="utf-8" />
...
function markdownToBasicHtml(md) {
	...
	for (const rawLine of lines) {
		...
		if (line.startsWith("# ")) out.push(`<h1>${escapeHtml(line.slice(2))}</h1>`);
			...
			else if (line.startsWith("```") || line === "") out.push("<br/>");
	...
	try {
		...
		if (!reportJsonPath) {
			const hint = [
				...
				"",
				"  npx playwright test --reporter=list,json=__tests__/reports/playwright-report.json,html=__tests__/reports/html",
				""
			].join("\n");
			...
			// Also throw to fail CI if desired:
			console.error(hint);
			throw new Error("JSON report not found");
		}

		// If the JSON is not in the preferred place, copy it there for consistency
