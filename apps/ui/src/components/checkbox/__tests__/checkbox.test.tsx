...
describe('components/checkbox', () => {
	describe('Layout', () => {
		...
		test('should render with label on top', () => {
			const { container } = render(<Checkbox label='label test' labelPosition='top' />);
			const labelElement = container.querySelector('label');
		...
		test('should render with label on right', () => {
			const { container } = render(<Checkbox label='label test' labelPosition='right' />);
			const labelElement = container.querySelector('label');
		...
		test('should render with label on bottom', () => {
			const { container } = render(<Checkbox label='label test' labelPosition='bottom' />);
			const labelElement = container.querySelector('label');
		...
		test('should render with label on left', () => {
			const { container } = render(<Checkbox label='label test' labelPosition='left' />);
			const labelElement = container.querySelector('label');
		...
		test('should render with custom icons', () => {
			render(<Checkbox icon={<MockIcon />} checkedIcon={<MockIcon fill />} value={true} />);
			const checkbox = screen.getByRole('checkbox');
		...
		test('should render indeterminate when value is null and icons are provided', () => {
			render(<Checkbox icon={<MockIcon />} checkedIcon={<MockIcon fill />} value={null} />);
			const checkbox = screen.getByRole<HTMLInputElement>('checkbox');
		...
		test('should render with TextProperties label with color', () => {
			const { container } = render(<Checkbox label={{ content: 'Colored label', color: 'red' }} value={true} />);
		...
		test('should render with TextProperties label with className', () => {
			const { container } = render(
				<Checkbox label={{ content: 'Custom class label', className: 'custom-label' }} value={true} />
			);
			const textElement = container.querySelector('.text');

			expect(textElement).toHaveClass('custom-label');
		});

		test('should render with ReactNode children', () => {
			render(
				<Checkbox value={true}>
	...
	describe('Events', () => {
		...
		test('should execute onChange handler when has custom icons and is clicked', () => {
			const handleChange = jest.fn();
			render(<Checkbox icon={<MockIcon />} checkedIcon={<MockIcon fill />} onChange={handleChange} />);
